    ...
    try:
        ...
        if raw_masks is None or raw_scores is None or len(raw_scores) == 0:
            empty_json = {
                ...
                "num_selected": 0,
                "selected_regions": [],
                "qwen_reason": "SAM3 found no candidate regions."
            }
        ...
        if len(candidate_regions) == 0:
            empty_json = {
                ...
                "num_selected": 0,
                "selected_regions": [],
                "qwen_reason": "SAM3 masks were empty after post-processing."
            }
        ...
        status = (
            ...
            f"Qwen selected {len(selected_regions)} region(s)."
        )
...
def explain_detection(image, prompt, detection_json_text):
    if QWEN_MODEL is None or QWEN_PROCESSOR is None:
        raise gr.Error("Qwen model failed to load.")
    if image is None:
        raise gr.Error("Please upload an image.")
    ...
    <div class="app-header">
        <div class="header-content">
            ...
            <div class="header-text">
                <h1>SAM3 + Qwen 3.5 — Image & Video Segmentation</h1>
                <div class="header-meta">
                    <span class="meta-badge">{SVG_CHIP} {SAM_MODEL_NAME}</span>
                  <!--    <span class="meta-sep"></span> -->
                  <!--  <span class="meta-badge">{SVG_CHIP} {MODEL_VL}</span> -->
                    <span class="meta-sep"></span>
                    <span class="meta-cap">SAM3 Proposals</span>
                    <span class="meta-sep"></span>
                    <span class="meta-cap">Qwen3.5 Filtering</span>
                    <span class="meta-sep"></span>
    ...
    with gr.Tabs():
        with gr.Tab("Image Detection (*Filter)"):
            gr.HTML(html_tab_intro(
                SVG_IMAGE,
                "Image Detection with SAM3 + Qwen 3.5",
                "SAM3 first proposes candidate masks and regions from your text prompt. Qwen 3.5 then filters those candidates and keeps only the regions that best match the request.",
                "Image mode: SAM3 proposes regions, Qwen filters final detections.",
            ))
            ...
            with gr.Row():
                with gr.Column(scale=1):
                    ...
                    detect_btn = gr.Button("Run SAM3 + Qwen Detection", variant="primary")
                ...
                with gr.Column(scale=1):
                    ...
                    gr.HTML(html_section_heading(SVG_OUTPUT, "Final Qwen-Filtered Detection"))
                    final_output = gr.Image(label="SAM3 + Qwen Result", height=300)
                    ...
                    gr.Markdown(
                        ...
                        - Click **"Run SAM3 + Qwen3.5 Detection"**:
                        - **Top Panel:** Candidate regions, **Bottom Panel:** Final filtered detections (Qwen3.5).
                        - **JSON Output:** Structured results including bounding boxes, scores, and labels.
                        - Click **"Explain Result"** to get a natural language explanations.
                        """
                    )
                ...
                with gr.Column(scale=1):
                    ...
                    gr.HTML(html_status_indicator(
                        "Pipeline: SAM3 proposes regions → Qwen filters relevant detections."
                    ))

                    gr.HTML(html_section_heading(SVG_TEXT, "Qwen Explanation"))
